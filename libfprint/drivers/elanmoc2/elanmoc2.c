                          ...
                          GError *error)
{
  ...
  else
  {
    ...
    if (transfer->actual_length > 0 &&
        ...
        transfer->buffer[0] != 0x98 &&
        transfer->buffer[0] != 0x00)
    {
    ...
    if (self->just_wiped)
    {
      self->just_wiped = FALSE;
      fpi_ssm_next_state_delayed(self->ssm, 10000);
    }
    else
    {
      fpi_ssm_next_state(self->ssm);
...
{
  return self->dev_type == ELANMOC2_DEV_0C5E ? ELANMOC2_USER_ID_MAX_LEN_0C5E : ELANMOC2_USER_ID_MAX_LEN;
}
...
elanmoc2_identify_run_state(FpiSsm *ssm, FpDevice *device)
{
  ...
  case IDENTIFY_CHECK_NUM_ENROLLED:
  {
    ...
    fp_info("Skipping identify, proceeding with enroll");
    elanmoc2_identify_verify_report(device, NULL, &error);
    elanmoc2_identify_verify_complete(device, NULL);
    fpi_ssm_mark_completed(g_steal_pointer(&self->ssm));
    break;
...
elanmoc2_enroll_run_state(FpiSsm *ssm, FpDevice *device)
{
  ...
  case ENROLL_CHECK_NUM_ENROLLED:
  {
    ...
    {
      fp_info("Sensor storage is full, wiping sensor first");
  ...
  case ENROLL_CHECK_COMMITTED:
  {
    ...
    if (data_in[1] != 0)
    {
      ...
      fp_warn("ELANMOC2: ignoring protocol error");
      g_clear_error(&error);
      fpi_ssm_next_state(self->ssm);
    }
    else
...
static const FpIdEntry elanmoc2_id_table[] = {
    ...
    {.vid = ELANMOC2_VEND_ID, .pid = 0x0c4d, .driver_data = ELANMOC2_ALL_DEV},
